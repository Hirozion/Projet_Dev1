...
int populate_packet_ds(const struct pcap_pkthdr *header, const u_char *packet, ETHER_Frame *custom_frame)
{
        ...
        if(ntohs(ethernet->ether_type) == ETHERTYPE_IP) 
        {
                ...
                {
                        //printf("\nUDP Handling\n");
                        custom_packet.ip_protocol = UDP_PROTOCOL;
                       	udp = (struct sniff_udp*)(packet + SIZE_ETHERNET + size_ip);

                        UDP_segment custom_udp;

                        custom_udp.source_port = ntohs(udp->uh_sport);
                        custom_udp.destination_port = ntohs(udp->uh_dport);
                        
                        custom_packet.data_UDP = custom_udp;
                        custom_frame->data = custom_packet;
                }
                if((int)ip->ip_p==ICMP_PROTOCOL)
                {
                	custom_packet.ip_protocol = ICMP_PROTOCOL; // modficiation 
                    custom_frame->data = custom_packet;
                }
                if((int)ip->ip_p==TCP_PROTOCOL)
                {
                		...
                		custom_packet.ip_protocol = TCP_PROTOCOL;
                        tcp = (struct sniff_tcp*)(packet + SIZE_ETHERNET + size_ip);
                        TCP_Segment custom_segment; // faire +- le meme avec udp
